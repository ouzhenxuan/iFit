...
// 侧边栏显示整个屏幕的宽度比
CGFloat const kLeftWidthScale = 320 / 375.0;
NSString * const kLeftViewWillDisAppear = @"kLeftViewWillDisAppear";
NSString * const kLeftViewWillAppear = @"kLeftViewWillAppear";
...
//    [[kAppDelegate window] addSubview:leftView];
    [[kAppDelegate window] insertSubview:leftView belowSubview:self.centerViewController.view];
    
    [self leftViewShowShadow];
...
- (void)leftViewShowShadow
{
    if (!self.leftView) {
        return;
    }
    self.leftView.layer.shadowColor = [UIColor whiteColor].CGColor;
    self.leftView.layer.shadowOffset = CGSizeMake(0, 0);
    self.leftView.layer.shadowOpacity = 0;
    self.leftView.layer.shadowRadius = 0;
...
- (void)resetShowType:(PSDrawerManagerShowType)showType {
    ...
    CGFloat scale = kLeftWidthScale / (1- kLeftWidthScale);
    switch (showType) {
        case PSDrawerManagerShowLeft: {
            [UIView animateWithDuration:0.2f animations:^{
                self.centerViewController.view.transform = rightScopeTransform;
                [kAppDelegate window].subviews.firstObject.tx = self.centerViewController.view.tx / scale;
            }];
...
- (void)handlePanAction:(UIPanGestureRecognizer *)sender {
    ...
    [kAppDelegate window].subviews.firstObject.tx = sender.view.tx / scale;
    // 3. 让平移的值不要累加
    ...
    if (sender.state == UIGestureRecognizerStateEnded) {
        [UIView animateWithDuration:0.2f animations:^{
            if (sender.view.left > kScreenWidth * 0.5) {// 展示leftview
                sender.view.transform = rightScopeTransform;
                [kAppDelegate window].subviews.firstObject.tx = sender.view.tx / scale;
                [self leftViewWillAppear];
            } else {
...
- (void)leftViewWillAppear
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLeftViewWillAppear object:self];
...
- (void)leftViewWillDisappear
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLeftViewWillDisAppear object:self];
}
